...
describe('MA presentation opening chapter', () => {
  ...
  it('uses the flat approach flow and reveals the original core-loop diagram', () => {
    ...
    expect(economyItems[0]).toHaveTextContent('Spin — Primary action and monetized energy.')
    expect(economyItems[1]).toHaveTextContent('Coin — Core progression currency.')
    expect(economyItems[2]).toHaveTextContent('Star — Status and progression signal.')
    expect(economyItems[3]).toHaveTextContent('Gem — Premium acceleration currency.')
    ...
    expect(mapSystems).toHaveAttribute('aria-expanded', 'true')
    expect(approachDiagram).toHaveClass('opacity-100')
    fireEvent.mouseEnter(screen.getByText('Research advanced play'))
    expect(mapSystems).toHaveAttribute('aria-expanded', 'false')

...
describe('DemoShell presentation contract', () => {
  ...
  it('defines a presentation mode without changing the standalone shell', () => {
    ...
    expect(shell).toContain("'--prototype-control-offset'")
    ...
    expect(css).toMatch(/\.prototypeControlsPresentation\s*{[^}]*position:\s*absolute;[^}]*left:\s*calc\(50% \+ var\(--prototype-control-offset\)\);/)
    expect(css).toMatch(/\.prototypeControlsPresentation\s*{[^}]*top:\s*50%;[^}]*transform:\s*translateY\(-50%\);/)

      ...
      <tbody>
        {rows.map((r, ri) => (
          <tr key={r.useCase} className="border-b border-charcoal/15">
            <td onMouseEnter={() => setHover(null)} style={{ backgroundColor: !hover && r.winner ? YELLOW : undefined }} className={`py-3 pr-4 font-sans text-[16px] ${r.winner || hover?.row === ri ? 'text-black' : 'text-charcoal'} ${(r.winner && !hover) || hover?.row === ri ? 'font-bold' : ''}`}>
